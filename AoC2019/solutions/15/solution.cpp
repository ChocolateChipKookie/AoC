...
std::string sourceDirectory = "../AoC2019/solutions/15";
...
std::vector<int> move_xy(std::pair<lli, lli> current_pos,
                         std::pair<lli, lli> pos) {
  if (current_pos == pos)
    return {};
  ...
  for (int i = 1; i <= 4; ++i) {
    std::pair<lli, lli> next = {current_pos.first + dx[i],
                                current_pos.second + dy[i]};
  ...
  while (true) {
    std::map<std::pair<lli, lli>, std::pair<std::pair<lli, lli>, int>>
        next_visit;
    ...
    for (auto visit : to_visit) {
      ...
      for (int i = 1; i <= 4; ++i) {
        std::pair<lli, lli> next = {visit.first.first + dx[i],
                                    visit.first.second + dy[i]};
        if (visited.find(next) == visited.end() &&
            map_.find(next) != map_.end() && map_[next] == ' ') {
...
void task_01() {
  ...
  while (true) {
    ...
    if (res == 1) {
      ...
      for (int i = 1; i <= 4; ++i) {
        std::pair<lli, lli> next = {undis.first.first + dx[i],
                                    undis.first.second + dy[i]};
...
int recursive_oxygen_fill(std::set<std::pair<lli, lli>> &visited,
                          std::pair<lli, lli> pos) {
  visited.insert(pos);
  if (map_[pos] == '#')
    return 0;
