...
private:
  ...
  public:
    explicit GridSpan(std::vector<T_data> &data, size_t start, size_t stride)
        : _data(data), _start(start), _stride(stride) {}
    T_data &operator[](size_t i) { return _data[_start + _stride * i]; }
...
public:
  ...
  Grid(size_t width, size_t height, T_data fill = 0)
      : _size{width, height}, _data(width * height, fill){};

  explicit Grid(std::array<size_t, 2> size, T_data fill = 0)
      : _size{size}, _data(size[0] * size[1], fill){};

  ///////////////////
  /// Data access ///
  ///////////////////
  [[nodiscard]] T_data &get(size_t i) { return _data.at(i); }
  [[nodiscard]] T_data &get(size_t x, size_t y) {
    return _data.at(get_index(x, y));
  }

  [[nodiscard]] T_data &operator()(size_t i) { return _data[i]; }
  [[nodiscard]] T_data &operator()(size_t x, size_t y) {
    return _data[get_index(x, y)];
  }
  [[nodiscard]] T_data get(size_t i) const { return _data.at(i); }
  [[nodiscard]] T_data get(size_t x, size_t y) const {
    return _data.at(get_index(x, y));
  }

  [[nodiscard]] T_data operator()(size_t i) const { return _data[i]; }
  ...
  [[nodiscard]] std::array<size_t, 2> size() const { return _size; }
  [[nodiscard]] size_t length() const { return _data.size(); }

  [[nodiscard]] const std::vector<T_data> &data() const { return _data; }

  [[nodiscard]] std::vector<T_data> &data() { return _data; }
