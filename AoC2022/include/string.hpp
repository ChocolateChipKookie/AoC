...
namespace kki
{
    ...
    private:
      ...
      string(const std::shared_ptr<std::vector<char>>& data, const char* begin, const char* end) : _begin(begin), _end(end), container(data){}
    ...
    public:
      ...
      explicit string_builder(size_t size=0) : _data(std::make_shared<std::vector<char>>()){
            _data->reserve(size);
        }
      string_builder(char t, size_t size) : _data(std::make_shared<std::vector<char>>()){
            _data->reserve(size);
      ...
      string_builder(size_t len, const char* data) : _data(std::make_shared<std::vector<char>>()){
            _data->reserve(len);
            std::copy(data, data + len, std::back_inserter(*_data));
        }
      explicit string_builder(std::shared_ptr<std::vector<char>>& data_container) : _data(data_container){}
        template<typename ...T_args>
        explicit string_builder(const char *format, T_args ...args) : _data(std::make_shared<std::vector<char>>()){
      ...
      std::shared_ptr<std::vector<char>> get_data_container()
        {
            return _data;
        }
