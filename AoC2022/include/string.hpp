...
public:
  ...
  string(size_t len, const char *data) {
    container = std::make_shared<std::vector<char>>();
  ...
  inline const char *data() const { return _begin; }
  ...
  void set_data(const std::shared_ptr<std::vector<char>> &data) {
    container = data;
    _begin = container->data();
  ...
  size_t hash() const { return hash(begin(), size()); }
  ...
  inline bool operator==(const string &other) const {
    return size() == other.size() &&
           (data() == other.data() || equal(data(), other.data(), size()));
  }
  ...
  template <typename T_add> string_builder operator+(T_add other);
  template <typename T_add> void operator+=(const T_add &other);
  ...
  std::vector<size_t, T_alloc> find_all(char element, size_t start = 0,
  ...
  template <typename T_alloc = std::allocator<size_t>>
  std::vector<size_t, T_alloc> find_all(const string &element, size_t start = 0,
                                        size_t elements = 0) const {
    return find_all(element.begin(), element.size(), start, elements);
  }
  template <typename T_alloc = std::allocator<size_t>>
  std::vector<size_t, T_alloc> find_all(const char *element, size_t start = 0,
                                        size_t elements = 0) const {
    return find_all(element, length(element), start, elements);
  }
  template <typename T_alloc = std::allocator<size_t>>
  std::vector<size_t, T_alloc> find_all(const char *element, size_t len,
                                        size_t start, size_t elements) const {
    std::vector<size_t, T_alloc> res;
    res.reserve(elements);
  ...
  std::vector<size_t, T_alloc> find_all_if(const T_pred &pred, size_t start = 0,
                                           size_t elements = 0) const {
    std::vector<size_t, T_alloc> res;
    res.reserve(elements);
  ...
  string trimmed() const { return l_trimmed().r_trim(); }
  ...
  std::vector<string> split(char delimiter, size_t splits = 0) const {
    std::vector<string> res;
    res.reserve(splits);
    iterator current = begin();
  ...
  std::vector<string> split(const char *delimiter, size_t len,
                            size_t splits) const {
    std::vector<string> res;
    res.reserve(splits);
    iterator current = begin();
  ...
  // If the string is the only string that uses that container there is no
  // reason to detach If the string is sharing the container, a new container
  // reference of fitting size is created and the content of the string is
  // copied and zero terminated Return: returns true if the string has been
  // detached
  bool detach() {
  ...
  bool shrink() {
    ...
    if ((data.size() - 1) != size()) {
      ...
      if (container.use_count() == 1) {
        size_t len = size();
        // If the data is not at the start of the container, copy it to the
        // start
        if (begin() != data.data()) {
  ...
  const char *cstr() {
    ...
    if ((*end()) != '\0') {
      // If the string is the only one that uses the container zero terminate it
      // in place
      if (container.use_count() == 1) {
  ...
  string clone() const {
    auto new_container = std::make_shared<std::vector<char>>();
    new_container->reserve(size());
    std::copy(begin(), end(), std::back_inserter(*new_container));
    return {new_container, new_container->data(),
            new_container->data() + new_container->size()};
  }
  ...
  template <typename T_stream> bool getline(T_stream &input_stream) {
  ...
  static inline size_t length(const char *cstr) { return strlen(cstr); }

private:
  // Private constructor for member functions
  string(const std::shared_ptr<std::vector<char>> &data, const char *begin,
         const char *end)
      : _begin(begin), _end(end), container(data) {}

  const char *_begin{nullptr}, *_end{nullptr};
  std::shared_ptr<std::vector<char>> container;
};
...
public:
  ...
  explicit string_builder(size_t size = 0)
      : _data(std::make_shared<std::vector<char>>()) {
    _data->reserve(size);
  }
  string_builder(char t, size_t size)
      : _data(std::make_shared<std::vector<char>>()) {
    _data->reserve(size);
    std::fill_n(std::back_inserter(*_data), size, t);
  }
  explicit string_builder(const char *cstr)
      : string_builder(string::length(cstr), cstr) {}
  explicit string_builder(const string &string)
      : string_builder(string.size(), string.begin()) {}
  string_builder(size_t len, const char *data)
      : _data(std::make_shared<std::vector<char>>()) {
    _data->reserve(len);
    std::copy(data, data + len, std::back_inserter(*_data));
  }
  explicit string_builder(std::shared_ptr<std::vector<char>> &data_container)
      : _data(data_container) {}
  ...
  void set_data_container(std::shared_ptr<std::vector<char>> &other) {
    _data = other;
  }
  std::shared_ptr<std::vector<char>> get_data_container() { return _data; }
  ...
  inline iterator begin() const { return _data->data(); }
  inline iterator end() const { return _data->data() + _data->size(); }
  inline char &back() { return _data->back(); }
  ...
  inline char operator[](size_t i) const { return get(i); }
  ...
  template <typename T_pr> string_builder &append(T_pr t) {
    return (*this) << t;
  }
  ...
  string_builder &operator<<(bool b) { return append(b ? "true" : "false"); }
  ...
  template <typename T_p> string_builder operator+(T_p p) {
    string_builder s;
  ...
  string to_string() { return string(_data->size(), _data->data()); }
  ...
  struct view {
  public:
    char &at(size_t i) { return _parent.at(_begin + i); }
    char &operator[](size_t i) { return at(i); }

    char *begin() { return _parent.begin() + _begin; }
    char *end() { return _parent.begin() + _end; }
    size_t size() const { return _end - _begin; }

    template <typename T_pr> string_builder &operator=(const T_pr &i) {
      ...
      if (len < size()) {
        ...
        _parent._data->erase(_parent._data->begin() + _begin + len,
                             _parent._data->begin() + _end);
      } else if (len >= size()) {
        // Replacement string is longer than the view
        std::copy(other.begin(), other.begin() + size(), begin());
        _parent._data->insert(_parent._data->begin() + _begin + size(),
                              other.begin() + size(), other.end());
      } else {
  ...
  private:
    ...
    view(string_builder &parent, size_t begin, size_t end)
  ...
  // Inserter view;
  view operator()(size_t begin) { return {*this, begin, begin}; }
...
private:
  ...
  static void format_recursion(string_builder &b, const char *format, T value,
                               T_args... args) {
...
template <typename T_add> string_builder string::operator+(T_add other) {
  string_builder s(*this);
  return s << other;
}

template <typename T_add> void string::operator+=(const T_add &other) {
...
template <typename T_stream> static string getline(T_stream &input_stream) {
  static std::string line;
...
std::ostream &operator<<(std::ostream &stream,
                         const kki::string_builder &builder) {
  return stream.write(builder.data(), builder.size());
}

namespace std {
template <> struct hash<kki::string> {
  std::size_t operator()(const kki::string &k) const { return k.hash(); }
};
} // namespace std
