...
std::vector<Position> &expand_positions(const Position &current, CharGrid &grid,
                                        bool inverted = false) {
...
struct Node {
  ...
  [[nodiscard]] int_t score() const { return distance + heuristic_distance; }
...
void task_02(CharGrid &data, Position end) {
  ...
  auto h_dist = [&valid_positions,
                 &h_dist_cache](const Position &pos) -> int_t {
    ...
    for (auto &position : valid_positions) {
      auto dist =
          std::abs(position[0] - pos[0]) + std::abs(position[1] - pos[1]);
