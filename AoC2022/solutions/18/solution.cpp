...
bool connected(const Position &p1, const Position &p2) {
  int_t distance = std::abs(p1[0] - p2[0]) + std::abs(p1[1] - p2[1]) +
                   std::abs(p1[2] - p2[2]);
  return distance == 1;
}
...
void task_02(std::vector<Position> &positions) {
  ...
  std::array<std::array<int_t, 3>, 6> directions{
      std::array<int_t, 3>{0, 0, 1}, std::array<int_t, 3>{0, 0, -1},
      std::array<int_t, 3>{0, 1, 0}, std::array<int_t, 3>{0, -1, 0},
      std::array<int_t, 3>{1, 0, 0}, std::array<int_t, 3>{-1, 0, 0}};
  ...
  auto reachable = [&](const Position &p, int_t count) -> bool {
    ...
    while (current_i < to_expand.size()) {
      ...
      for (const auto &direction : directions) {
        Position npos = {current[0] + direction[0], current[1] + direction[1],
...
int main() {
  ...
  for (const auto &line : lines) {
    ...
    positions.push_back(
        Position{parse(tokens[0]), parse(tokens[1]), parse(tokens[2])});
  }
