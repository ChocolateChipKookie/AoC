...
void print(const std::vector<Layer> &layers, const Position &reference,
           size_t rock_id) {
  std::set<Position> rock_positions;
  for (const auto &position : rocks[rock_id]) {
    rock_positions.insert(
        {reference[0] + position[0], reference[1] + position[1]});
  }
...
bool collides(const Position &reference, size_t rock_id,
