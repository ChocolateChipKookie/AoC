...
void solution(){
    ...
    for(auto& line : data){
        ...
        for (size_t i = 1; i < tokens.size(); ++i){
            ...
            int64 length = std::abs(next[0] - current[0]) + std::abs(next[1] - current[1]);
            Position direction = {(next[0] - current[0]) / length, (next[1] - current[1]) / length};
            for (int d = 0; d < length; ++d){
                blocks.insert({current[0] + direction[0] * d, current[1] + direction[1] * d});
            }
            current = next;
        }
        blocks.insert(current);
