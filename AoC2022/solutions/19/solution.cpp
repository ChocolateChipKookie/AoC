...
struct Blueprint {
  explicit Blueprint(const kki::string &blueprint) {
    ...
    std::from_chars(blueprint.begin() + id_start, blueprint.begin() + id_end,
                    id);

    std::from_chars(tokens[6].begin(), tokens[6].end(),
                    costs[Resource::Ore][Resource::Ore]);
    std::from_chars(tokens[12].begin(), tokens[12].end(),
                    costs[Resource::Clay][Resource::Ore]);
    std::from_chars(tokens[18].begin(), tokens[18].end(),
                    costs[Resource::Obsidian][Resource::Ore]);
    std::from_chars(tokens[21].begin(), tokens[21].end(),
                    costs[Resource::Obsidian][Resource::Clay]);
    std::from_chars(tokens[27].begin(), tokens[27].end(),
                    costs[Resource::Geode][Resource::Ore]);
    std::from_chars(tokens[30].begin(), tokens[30].end(),
                    costs[Resource::Geode][Resource::Obsidian]);
...
int_t evaluate(Blueprint &b, int_t steps, int_t current_best) {
  ...
  int_t max_possible = b.resources[Resource::Geode] +
                       b.robots[Resource::Geode] * steps + steps * steps / 2;
