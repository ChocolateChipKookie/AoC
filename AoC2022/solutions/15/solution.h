...
int_t sweep(std::vector<std::array<int_t, 2>>& ranges){
    std::sort(ranges.begin(), ranges.end(), [](const auto& r1, const auto& r2){ return r1[0] < r2[0];});
    int_t occupied = 0;
...
void create_ranges(std::vector<Sensor>& sensors, std::vector<std::array<int_t, 2>>& container, int_t y, int_t x_start, int_t x_end){
    container.clear();

    for (auto& sensor : sensors){
        auto& beacon = sensor.beacon;
        int_t bs_distance = std::abs(sensor.x - beacon.x) + std::abs(sensor.y - beacon.y);
...
void task_01(std::vector<Sensor>& sensors, int_t y){
    ...
    create_ranges(sensors, ranges, y, std::numeric_limits<int_t>::min(), std::numeric_limits<int_t>::max());
...
void solution(){
    ...
    for (auto& line : data){
        ...
        sensors.push_back(
                Sensor {
                        .x = parse(line.substr(x_sensor_s, x_sensor_e)),
                        .y = parse(line.substr(y_sensor_s, y_sensor_e)),
                        .beacon = Beacon{
                                .x = parse(line.substr(x_beacon_s, x_beacon_e)),
